...
**Methodologies:** Execution playbooks (`templates/diagnose-execution-playbooks.md`); JSON sidecars beside state (problem spec, first-principles, hypotheses, MECE tree, five-whys chains, 20-row technique coverage, barriers when high-severity); orchestrator gates at steps 4–5–7 with combined **DIAGNOSE ARTIFACT GATE**; IS/IS-NOT; Cynefin; change analysis; fishbone via hypothesis register (≥10 candidates, ≥4 categories); FMEA via `fmea_score.py`; but-for / Pareto; git hotspots and logs; solutions only for confirmed causes.
