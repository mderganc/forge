...
- **Runtime:** design startup checks Beads (`prompts/develop/startup.md`). Skill state includes a `beads_available` flag in `scripts/shared/orchestrator.py`, but whether Beads is used is driven by prompts and `project.md` (“beads: available/unavailable”), not by automatic detection in the orchestrator.
...
After a new `forge-next` release on PyPI, upgrade with `pipx upgrade forge-next` (or reinstall with `pipx install forge-next --force`). Pin a specific version when reproducibility matters, for example `pipx install 'forge-next==0.19.0'` (see `project.version` in `pyproject.toml`).

---

## Commands in your apps

All **14** workflows are defined in [`integrations/spec/commands.json`](integrations/spec/commands.json). They split into **core** delivery commands (linear pipeline + ship) and **helper** commands (meta, resume, environment, graph).

### Core commands (9)

| Workflow | Cursor / Claude | Codex |
|----------|-----------------|-------|
| **Sketch** — organize intent before design (optional) | `/forge:sketch` | `$forge:sketch` |
| **Design** — discovery, requirements, options; medium/large scope requires a **named** spec at `docs/forge/specs/YYYY-MM-DD-<slug>-design.md` | `/forge:design` | `$forge:design` |
...
| Structured code review (includes structural probe summary on final step) | `/forge:code-review` | `$forge:code-review` |
...
**Terminal:** same subcommands as `forge <subcommand> …` (for example `forge design --step 1`, `forge graphify refresh`). **`forge develop`** remains a **deprecated alias** for `forge design` (stderr warning).
...
When intent is still fuzzy, run **sketch** before design (see [Sketch](#sketch-before-design)).
...
### Sketch (before design)

- **Cursor / Claude:** `/forge:sketch`
- **Codex:** `$forge:sketch`

**Sketch organizes intent** when the problem, constraints, or terminology are still fuzzy: one question at a time with a recommended answer, logged in **`memory/sketch-decisions.md`**. It is **not** solution brainstorming (that is design) and **not** the design spec file. Optional **`--with-domain-docs`** updates `CONTEXT.md` (glossary) and sparse `docs/adr/` per `templates/CONTEXT-FORMAT.md` and `templates/ADR-FORMAT.md`.

Default handoff: **design**. Protocol: `templates/sketch-protocol.md`.

### Design

- **Cursor / Claude:** `/forge:design`
- **Codex:** `$forge:design`

**Design is a back-and-forth with you:** surface opportunities (not only bugs), brainstorm and refine requirements, and generate multiple solution directions before anything is “locked.” The step sequence structures that conversation—short rounds of questions, options, and gates—not a one-shot report.
...
**Named design spec:** For **medium** and **large** scope, design produces **`docs/forge/specs/YYYY-MM-DD-<slug>-design.md`** before handoff to plan — not sketch.

**Scope tiers and design spec:** Scope assessment can set **trivial / medium / large** (persisted as **`memory/design-scope.json`** under the Forge runtime root, e.g. `.codex/forge/`; legacy **`memory/develop-scope.json`** still read). For **medium** and **large**, step 7 enforces a **design-spec gate**: **`.design-spec-gate.json`** beside the design state file must record a real spec path, completion booleans, and user approval (legacy **`.develop-spec-gate.json`** still read). Strict bypass is opt-in via `forge design --step 7 --allow-spec-incomplete` with **`--spec-override-reason`** and **`--spec-override-follow-up`** (optional **`--spec-override-requested-by`**). Template: `templates/design-spec.md`.
...
Evidence-led root-cause analysis and reporting. When the workflow classifies the fix as **`large`** (systemic / needs design before planning), the closing handoff menu defaults to **design** first, then plan; for **`complex`** (broad but plannable), it defaults to **plan**.
